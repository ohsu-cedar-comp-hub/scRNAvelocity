...
for (i in 1:dim(run2process)[1]){
		samples_seurat <- c(samples_seurat,as.character(run2process[i,2]))
}		
seuratObj <- readRDS(seurat)
seuratObj <- subset(seuratObj, cells = Cells(seuratObj)[which(endsWith(Cells(seuratObj), samples_seurat))])
...
for (clust in unique_clusters){
	...
	for (i in 1:length(genes_of_interest)){
		...
		file_path <- paste(base_path,"..","Analyze/figures",sprintf("scvelo_scatter_gene_cluster_%s_%s.png",clust,cur_gene),sep="/")
		if(file.exists(file_path)){
			cat("\n\n#### ",cur_gene,"\n\n")
			cat('\n![Scatter](',file_path,')\n\n')
		}
