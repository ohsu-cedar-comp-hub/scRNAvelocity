...
params:
  ...
  seurat: "seurat_path"
  genes: "RUNX1 CD74 MIF FOS CCL2 PU.1 TLR4 TLR2 CD44 SRC PI3K AKT TEN JAB1 CXCL8 MAPK ERK SPI1 STAT3 STAT5 NFKb CXCR1/2 CXCR1 CXCR2 JUN GATA1"
  contrast: "None"
...
packages_to_work <- c("Seurat","dplyr","ggplot2","scales","tidyr","gridExtra","xtable","rlang","plotly","ggrepel","ggpubr","DT","data.table","extrafont")
...
color_tsv_file <- read.delim(params$color_tsv)
cluster_colors <- unlist(color_tsv_file[1,-1])
names(cluster_colors) <- gsub("\\.","-",names(cluster_colors)) #revert . to - as this was performed in python
...
```{r }
message(sprintf("loading observational data: %s",paste(getwd(),scvelo_path,sep="/")))
#read in a meta data file that contains information about each cell
scvelo <- read.delim(scvelo_path, sep = "\t", fileEncoding = 'utf-8')
...
clusters_of_interest <- params$clusters_of_interest
clusters_of_interest <- unlist(clusters_of_interest)

color_names <- unlist(params$color_names)
color_hex <- unlist(params$color_hex)
...
file_path <- file.path(fig_path,"scvelo_scvelo_stream.png")
...
file_path <- file.path(fig_path,"scvelo_scatter_confidence.png")
if(file.exists(file_path)){   knitr::include_graphics(path = file_path) } 
```
...
file_path <- file.path(fig_path,"scvelo_latent_time.pdf")
if(file.exists(file_path)){   knitr::include_graphics(path = file_path) } 
```
...
```{r error = FALSE}
message(sprintf("current directory: %s",paste(getwd())))
...
for (clust in sort(unique_clusters)){
    ...
    fig_path <- file.path(out_dir,"violin_files","figures","seurat_plots",gsub("/",".",clust))
    
    dir.create(fig_path, showWarnings = FALSE, recursive = TRUE)
    if (file.exists(fig_path)){
        message(sprintf("available directory: %s",fig_path))
    }
	for (gene in genes_of_interest){
		...
		# violin stat plot
		seurat_violin_file_path <- file.path(getwd(),fig_path,sprintf("VlnPlot_%s_%s_exp.png",gsub("/",".",clust),gene))
		if(!(file.exists(seurat_violin_file_path))){
			p_plot <- try(pplot <- VlnPlot_stats(sub_seurat, group_by = contrast, gene_signature = gene))
			if(class(p_plot) != "try-error"){
				ggsave(filename = basename(seurat_violin_file_path), plot = p_plot, path = dirname(seurat_violin_file_path), device = "png")
			}
		}
		# feature plot
		seurat_feature_plot_file_path <- file.path(getwd(),fig_path,sprintf("FeaturePlot_%s_%s_exp.png",gsub("/",".",clust),gene))
		if(!(file.exists(seurat_feature_plot_file_path))){
			p_plot <- try(pplot <- FeaturePlot(sub_seurat, features = gene, split.by = contrast))
			if(class(p_plot) != "try-error"){
				ggsave(filename = basename(seurat_feature_plot_file_path), plot = p_plot, path = dirname(seurat_feature_plot_file_path), device = "png")
			}
		}
	}
...
for (clust in sort(unique_clusters)){
	...
	fig_path <- file.path(out_dir,"bar_plots","figures","seurat",gsub("/",".",clust))
    dir.create(fig_path, showWarnings = FALSE, recursive = TRUE)
	file_path <- file.path(fig_path,sprintf("bar_plot_%s_clust.png",clust))
	...
	for (i in 1:length(genes_of_interest)){
		...
		file_path <- file.path(out_dir,"Velo_heat_files",sprintf("%s/velocity_%s_heatmap.png",gsub("/",".",clust),cur_gene))
		if(file.exists(file_path)){
		...
		#Phase plot
		file_path <- file.path(out_dir,sprintf("figures/scvelo_%s",gsub("/",".",clust)),sprintf("scatter_gene_%s_cluster.pdf",cur_gene))
		if(file.exists(file_path)){
		...
		# expression plot
        file_path <- file.path(getwd(),fig_path,sprintf("VlnPlot_%s_%s_exp.png",gsub("/",".",clust),gene))
		if(file.exists(file_path)){
			
			cat('\n![Violin Plot](',file_path,')\n\n')
		}
		#feature plot
		file_path <- file.path(getwd(),fig_path,sprintf("FeaturePlot_%s_%s_exp.png",gsub("/",".",clust),gene))
		if(file.exists(file_path)){
        ...
        #figures/{}/violin_genes_{}_{}.png #
        #fig_path <- file.path(out_dir,"violin_files")
		#file_path <- file.path(fig_path,sprintf("%s/violin_genes_%s_%s.png",gsub("/",".",clust),"velocity",cur_gene))
		#if(file.exists(file_path)){
		#	cat('\n![Violin](',file_path,')\n\n')
		#}
...
```{r ta_vel, warning = F}
v_table <- read.delim(file.path(out_dir,"tsv_files","cluster_condition_velocity.tsv"))
v_table <- v_table[,-1]
create_dt(v_table)
```

### Wilcoxon rank sum test for RNA expression and Velocity:   HD vs FPD {.tabset .tabset-fade .tabset-pills}
...
for (clust in sort(unique_clusters)){
	...
	DE_path <- file.path(out_dir,"tsv_files",sprintf("%s/wilcoxon_%s_RNA_velocity_merged.tsv",gsub("/",".",clust),"HD"))
...
fig_path = file.path(out_dir,"heatmap_files","figures")
file_path <- file.path(fig_path,"heatmapscanpy_Mu_heatmap.pdf")
if(file.exists(file_path)){
...
file_path <- file.path(fig_path,"heatmapscanpy_Ms_heatmap.pdf")
if(file.exists(file_path)){
...
file_path <- file.path(out_dir,"figures","scvelo_PAGA.pdf")
if(file.exists(file_path)){
	cat('\n![PAGA](',file_path,')\n\n')
}
```
...
# Pseudotime Percent Cluster  {.tabset .tabset-fade .tabset-pills}

## Line {.tabset .tabset-fade .tabset-pills}

### Diffusion

```{r r_line1, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","pseudotime_dpt_r.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```

### Latent Time

```{r r_line2, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","pseudotime_latent_time_r.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```
...
```{r r_line3, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","pseudotime_velocity_pseudotime_r.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```
...
```{r py_line1, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","bar_pseudotime_dpt_lineplot.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```
...
```{r py_line2, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","bar_pseudotime_latent_time_lineplot.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```
...
```{r py_line3, warning = F, results = 'asis'}
file_path <- file.path(out_dir,"figures","bar_pseudotime_velocity_pseudotime_lineplot.pdf")
if(file.exists(file_path)){
	cat('\n![Pseudoline](',file_path,')\n\n')
}
```
