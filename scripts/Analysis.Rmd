...
Color_hex <- seuratObj@misc$colors.ls$tp_gt$tp_gt.col
names(Color_hex) <- unname(sapply(names(Color_hex), function(x) { paste(unlist(strsplit(x,"_"))[2],unlist(strsplit(x,"_"))[1],sep = "_")}))
Order_plot <- names(Color_hex)
...
for (clust in unique_clusters){
	...
	if(class(top_velocity) != "try-error"){
		GOI[[as.character(clust)]] <- sort(unique(c(genes_of_interest,as.character(head(top_velocity[,2],n=10)),as.character(head(top_velocity[,3],n=10)),as.character(tail(top_velocity[,2],n=10)),as.character(tail(top_velocity[,3],n=10)))))
	} else {
		GOI[[as.character(clust)]] <- genes_of_interest
	}
...
for (clust in unique_clusters){
	...
	file_path <- sprintf("%s/../Analyze/top_%s_velocity_genes.tsv",base_path,gsub("/",".",clust))
	top_velocity <- try(read.delim(file_path))
	if (class(top_velocity) != "try-error"){
		print(kable(top_velocity))
	}
}
```
